...
exports.sched = function(req, res){	
	dbRows("SELECT * from sensors",function(sensors){ 
		var times = [];
...
function dbConnect(){
	...
	return new sqlite3.Database('/usr/share/csServ/de.sqlite');
}
...
function portWrite(port){
    if(port.alive === false) port.open();
	else {
        if(!g_alreg[port.macid]) g_alreg[port.macid] = 0;
        var outStr = String.fromCharCode(1) + daySec() + ":" + g_alreg[port.macid] + ":" + g_next4.join(":") + String.fromCharCode(10);
        //cl('writing > ' + port.path);
...
function portReply(port,reply){
    cl(daySec()+":"+port.path+' RECV:' + reply.replace("\r\n",""));     
	reply = reply.replace('\u0001','').split('|');
	//console.log(reply);	
